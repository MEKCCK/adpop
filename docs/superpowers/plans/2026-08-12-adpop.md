...
- 退出码：`0`=用户关闭，`1`=超时，`2`=参数错误，`3`=无显示后端，`4`=图片加载失败（降级纯文字）
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CloseReason { Closed, TimedOut }
...
impl PopupSpec {
    pub fn validate(&self) -> Result<(), String> {
        if self.text.is_empty() && self.image.is_none() {
            return Err("nothing to show: provide --text and/or --image".to_string());
        }
        if self.count == 0 {
